...
(function (window) {
  ...
  async function savePublicationCentral(entry, pdfDataUrl, removePdf) {
    try {
      ...
      if (window.JPLAuth && window.JPLAuth.getAccessToken) {
        var accessToken = await window.JPLAuth.getAccessToken();
      ...
      console.warn('Central API save error:', err);
    }
  ...
  async function deletePublicationCentral(id) {
    try {
      ...
      var res = await fetch('/api/publications/' + encodeURIComponent(id), { method: 'DELETE', headers: headers });
      if (res.ok) {
